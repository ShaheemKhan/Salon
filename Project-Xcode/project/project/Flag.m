...
- (NSString*)flag:(Post *)p  {
    NSString *pid  = [[p primaryID] stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
    s = [NSString stringWithFormat: @"http://localhost:8080/social/flag?id=%@", pid];

    NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
    NSMutableArray *a = [defaults objectForKey:@"flags"];
    if (a == nil) {
        a = [[NSMutableArray alloc] init];
        [a addObject: pid];
        [defaults setObject:a forKey:@"flags"];
...
{
    if ([elementname isEqualToString:@"flags"])
    {
